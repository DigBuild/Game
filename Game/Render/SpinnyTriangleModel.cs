...
namespace DigBuild.Render
{
    public sealed class SpinnyTriangleModel : IBlockModel
    {
        ...
        public void AddDynamicGeometry(GeometryBufferSet buffers)
        {
            ...
            var normal = Vector3.TransformNormal(Vector3.Normalize(new Vector3(1, 0, 1)), matrix);
            
            var v1 = new SimpleVertex(new Vector3(0.5f, 1f, 0.5f), normal, _sprite.GetInterpolatedUV(0.5f, 1));
            var v2 = new SimpleVertex(Vector3.Transform(new Vector3(0, 0, 0), matrix), normal, _sprite.GetInterpolatedUV(0, 0));
            var v3 = new SimpleVertex(Vector3.Transform(new Vector3(1, 0, 1), matrix), normal, _sprite.GetInterpolatedUV(1, 0));
